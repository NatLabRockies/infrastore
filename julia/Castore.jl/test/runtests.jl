...
@testset "Castore.jl smoke" begin
    ...
    @test_throws Castore.NotFoundError get_time_series(store, key)
end
...
@testset "attribute-based metadata + hash access" begin
    ...
    @test_throws Castore.NotFoundError get_metadata(
        store, owner, Component, "load"; resolution=resolution, features=feats
    )
end

@testset "Castore.jl persistent round-trip" begin
    mktempdir() do dir
        ...
        store = Castore.open_store(path; read_only=true)
        try
            counts = get_counts(store)
            @test counts.static_time_series == 1
...
@testset "transform_single_time_series! derives DST read as Deterministic" begin
    ...
    @test_throws Castore.NotFoundError get_time_series(
...
@testset "list_keys filters (owner, type, name, resolution, features)" begin
    ...
    @test length(list_keys(store; time_series_type=Castore.CASTORE_TYPE_SINGLE)) == 4
    ...
    @test Set(get_resolutions(store; time_series_type=Castore.CASTORE_TYPE_SINGLE)) ==
        Set([Millisecond(Minute(5)), Millisecond(Hour(1))])
    @test isempty(
        get_resolutions(store; time_series_type=Castore.CASTORE_TYPE_DETERMINISTIC)
    )
...
@testset "time_series_counts and list_owner_ids" begin
    ...
    @test list_owner_ids(
        store, Component; time_series_type=Castore.CASTORE_TYPE_DETERMINISTIC
    ) == [1]
...
@testset "check_static_consistency and filtered get_forecast_parameters" begin
    ...
    @test_throws Castore.IntegrityError check_static_consistency(store)
...
@testset "Deterministic and DeterministicSingleTimeSeries are mutually exclusive" begin
    ...
    @test_throws Castore.InvalidParameterError add_time_series!(
        store,
        401,
        "Generator",
        Component,
    ...
    @test_throws Castore.InvalidParameterError transform_single_time_series!(
        store2, hor, ivl
    )
end
...
@testset "get_time_series_keys empty owner" begin
    store = Store(in_memory=true)
    @test get_time_series_keys(store, 999, Component) == Castore.TimeSeriesKey[]
end
...
@testset "compression policies round-trip" begin
    for (compression, level, shuffle) in
        ...
        mktempdir() do dir
            ...
            let store = Castore.open_store(path; read_only=false)
...
@testset "AddBatch all-or-nothing rollback" begin
    ...
    add_time_series!(batch, 800, "Generator", Component, ts)
    @test_throws Castore.DuplicateTimeSeriesError add_time_series_bulk!(store, batch)
    @test length(batch) == 0
...
@testset "StaticReader: columnar reads across dtypes and shapes" begin
    ...
    @test_throws Castore.InvalidParameterError static_read!(r, t0 + Minute(30))
end
...
@testset "ForecastReader: windows incl. multidim element shape" begin
    ...
    @test_throws Castore.InvalidParameterError forecast_read!(r, t0 + Hour(1))
end
...
@testset "Phase 2 additions: units, time_range, discovery, rename, bulk dispatch" begin
    ...
    @test isempty(get_intervals(store; time_series_type=Castore.CASTORE_TYPE_SINGLE))
    ...
    rk = resolve_forecast_key(
        store, 2, Component, "fc", Castore.CASTORE_TYPE_ABSTRACT_DETERMINISTIC
    )
...
@testset "Round-2 ABI: metadata element_shape/features, bulk remove" begin
    ...
    fmd = get_forecast_metadata(
        store, 2, Component, "fc", Castore.CASTORE_TYPE_DETERMINISTIC; features=feats
    )
    ...
    @test_throws Castore.NotFoundError remove_time_series!(store, [kp, kf])
...
@testset "Round-2 Julia idioms: Base interface, do-block, time_range, persist!" begin
    ...
    @test Castore.IOError <: Castore.TimeSeriesException
...
@testset "Supplemental-attribute associations" begin
    ...
    @test_throws Castore.DuplicateAssociationError add_supplemental_attribute_association!(
    ...
    @test_throws Castore.DuplicateAssociationError replace_supplemental_attribute_component_id!(
        store, 6, 5
    )
...
@testset "Parent/child associations" begin
    ...
    @test_throws Castore.DuplicateAssociationError add_parent_child_association!(
    ...
    @test_throws Castore.ReadOnlyStoreError add_parent_child_association!(
        reopened, edge(9, 900)
    )
    close!(reopened)
