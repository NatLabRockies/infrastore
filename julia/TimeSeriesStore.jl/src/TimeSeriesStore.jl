...
function _jll_library_path()
    ...
    return if isdefined(mod, :libtime_series_store_ffi)
...
const TS_ERR_DUPLICATE_ASSOCIATION = Int32(10)
...
function Base.showerror(io::IO, e::TimeSeriesException)
    return print(io, "TimeSeriesStore.", typeof(e).name.name, ": ", e.msg)
end

function _last_error_message()
    needed = Ref{UInt64}(0)
    ccall(
        (:ts_last_error_message, lib_path()),
        Int32,
...
function _check(code::Int32)
    ...
    elseif code == TS_ERR_DUPLICATE_ASSOCIATION
        throw(DuplicateAssociationError(msg))
    elseif code == TS_ERR_INVALID_PARAMETER ||
        code == TS_ERR_INVALID_UTF8 ||
        code == TS_ERR_NULL_POINTER
...
function Store(;
    ...
    code = ccall(
        (:ts_store_create_with_compression, lib_path()),
        Int32,
...
function _take_cstr(ptr::Ptr{Cchar})
    ...
    s = unsafe_string(ptr)
    ccall((:ts_string_free, lib_path()), Cvoid, (Ptr{Cchar},), ptr)
    return s
end
...
function add_time_series!(
    ...
    out_key = Ref{Ptr{Cvoid}}(C_NULL)
    code = ccall(
        (:ts_store_add_non_sequential, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function get_metadata(
    ...
    code = ccall(
        (:ts_store_get_metadata, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function get_forecast_metadata(
    ...
    out_fj_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_get_forecast_metadata, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function get_probabilistic_metadata(
    ...
    out_fj_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_get_probabilistic_metadata, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function resolve_forecast_key(
    ...
    out_key = Ref{Ptr{Cvoid}}(C_NULL)
    code = ccall(
        (:ts_store_resolve_forecast_key, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function get_array_by_hash(
    ...
    code = ccall(
        (:ts_store_get_array_by_hash, lib_path()),
        Int32,
    ...
    bytes = copy(raw)
    ccall(
        (:ts_buffer_free_u8, lib_path()), Cvoid, (Ptr{UInt8}, UInt64), out_data[], out_len[]
    )
...
function count_array_references(store::Store, data_hash::Vector{UInt8})
    ...
    code = ccall(
        (:ts_store_count_array_references, lib_path()),
        Int32,
...
function has_time_series(
    ...
    out = Ref{Bool}(false)
    code = ccall(
        (:ts_store_has_by_attrs, lib_path()),
        Int32,
...
function has_for_owner(
    ...
    code = ccall(
        (:ts_store_has_for_owner, lib_path()),
        Int32,
...
function remove_time_series!(
    ...
    features_json = isempty(features) ? C_NULL : pointer(JSON.json(features))
    code = ccall(
        (:ts_store_remove_by_attrs, lib_path()),
        Int32,
...
function _bulk_single(result::Ptr{Cvoid}, idx::Integer, name::AbstractString)
    ...
    _check(
        ccall(
            (:ts_bulk_result_get_single, lib_path()),
            Int32,
            (
                Ptr{Cvoid},
                UInt64,
...
function _bulk_non_sequential(result::Ptr{Cvoid}, idx::Integer, name::AbstractString)
    ...
    _check(
        ccall(
            (:ts_bulk_result_get_non_sequential, lib_path()),
            Int32,
            (
                Ptr{Cvoid},
                UInt64,
...
function _bulk_forecast(
    ...
    _check(
        ccall(
            (:ts_bulk_result_get_forecast, lib_path()),
            Int32,
            (
                Ptr{Cvoid},
                UInt64,
...
function bulk_read(
    ...
    try
        for i in 1:n
            ...
            _check(
                ccall(
                    (:ts_bulk_result_item_type, lib_path()),
                    Int32,
...
function get_time_series(
    ...
    tr_present, tr_start, tr_end = _time_range_args(time_range)
    code = ccall(
        (:ts_store_get_non_sequential, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Ptr{Cvoid},
...
function _make_key(
    ...
    out_key = Ref{Ptr{Cvoid}}(C_NULL)
    code = ccall(
        (:ts_make_key_from_attrs, lib_path()),
        Int32,
...
function _get_association(store::Store, key::TimeSeriesKey)
    ...
    code = ccall(
        (:ts_store_get_association, lib_path()),
        Int32,
...
function get_time_series_keys(
    ...
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_get_time_series_keys, lib_path()),
        Int32,
...
function _type_for_code(code::Integer)
    if code == TS_TYPE_SINGLE
        SingleTimeSeries
    elseif code == TS_TYPE_NON_SEQUENTIAL
        NonSequentialTimeSeries
    elseif code == TS_TYPE_DETERMINISTIC
        Deterministic
    elseif code == TS_TYPE_DETERMINISTIC_SINGLE
        DeterministicSingleTimeSeries
    elseif code == TS_TYPE_PROBABILISTIC
        Probabilistic
    elseif code == TS_TYPE_SCENARIOS
        Scenarios
    else
...
function list_time_series(
    ...
    json = _filter_probe(
        ...
        (buf, cap, out_len) -> ccall(
            (:ts_store_list_time_series, lib_path()),
            Int32,
            (
                Ptr{Cvoid},
                Bool,
...
function list_owner_types(
    ...
    json = _filter_probe(
        store,
        (buf, cap, out_len) -> ccall(
            (:ts_store_list_owner_types, lib_path()),
            Int32,
            (
                Ptr{Cvoid},
                Bool,
...
function remove_by_filter!(
    ...
    code = ccall(
        (:ts_store_remove_by_filter, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Bool,
...
function list_array_groups(
    ...
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_list_array_groups, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Bool,
...
function counts_by_type(store::Store)
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_counts_by_type, lib_path()),
        Int32,
...
function num_distinct_arrays(store::Store)
    out = Ref{Int64}(0)
    code = ccall(
        (:ts_store_num_distinct_arrays, lib_path()),
        Int32,
        (Ptr{Cvoid}, Ref{Int64}),
        store.handle,
        out,
...
function time_series_counts(store::Store)
    ...
    d = Ref{Int64}(0)
    code = ccall(
        (:ts_store_counts_detailed, lib_path()),
        Int32,
...
function list_owner_ids(
    ...
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_list_owner_ids, lib_path()),
        Int32,
...
function static_summary(store::Store)
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_static_summary, lib_path()),
        Int32,
        (Ptr{Cvoid}, Ptr{UInt8}, UInt64, Ref{UInt64}),
        store.handle,
        C_NULL,
...
function forecast_summary(store::Store)
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_forecast_summary, lib_path()),
        Int32,
        (Ptr{Cvoid}, Ptr{UInt8}, UInt64, Ref{UInt64}),
        store.handle,
        C_NULL,
...
function add_supplemental_attribute_association!(
    ...
    _check(
        ccall(
            (:ts_store_add_supplemental_attribute_association, lib_path()),
            Int32,
...
function add_supplemental_attribute_associations!(
    ...
    return _assoc_count_out(
        out -> ccall(
            (:ts_store_add_supplemental_attribute_associations, lib_path()),
            Int32,
...
function has_supplemental_attribute_association(
    ...
    return _assoc_bool_out(
        out -> ccall(
            (:ts_store_has_supplemental_attribute_association, lib_path()),
            Int32,
...
function list_supplemental_attribute_associations(
    ...
    json = _filter_probe(
        store,
        (buf, cap, out_len) -> ccall(
            (:ts_store_list_supplemental_attribute_associations, lib_path()),
            Int32,
...
function list_supplemental_attribute_ids(
    ...
    json = _filter_probe(
        store,
        (buf, cap, out_len) -> ccall(
            (:ts_store_list_supplemental_attribute_ids, lib_path()),
            Int32,
            (Ptr{Cvoid}, Cstring, Ptr{UInt8}, UInt64, Ref{UInt64}),
            store.handle,
            filter_json,
...
function list_components_with_attributes(
    ...
    json = _filter_probe(
        store,
        (buf, cap, out_len) -> ccall(
            (:ts_store_list_components_with_attributes, lib_path()),
            Int32,
            (Ptr{Cvoid}, Cstring, Ptr{UInt8}, UInt64, Ref{UInt64}),
            store.handle,
            filter_json,
...
function remove_supplemental_attribute_associations!(
    ...
    return _assoc_count_out(
        out -> ccall(
            (:ts_store_remove_supplemental_attribute_associations, lib_path()),
            Int32,
            (Ptr{Cvoid}, Cstring, Ref{UInt64}),
            store.handle,
            filter_json,
...
function replace_supplemental_attribute_component_id!(
    ...
    return _assoc_count_out(
        out -> ccall(
            (:ts_store_replace_supplemental_attribute_component_id, lib_path()),
            Int32,
...
function _supplemental_count(store::Store, filter_json, kind::Integer)
    ...
    _check(
        ccall(
            (:ts_store_count_supplemental_attribute_associations, lib_path()),
            Int32,
...
function supplemental_attribute_counts_by_type(store::Store)
    json = _filter_probe(
        store,
        (buf, cap, out_len) -> ccall(
            (:ts_store_supplemental_attribute_counts_by_type, lib_path()),
            Int32,
            (Ptr{Cvoid}, Ptr{UInt8}, UInt64, Ref{UInt64}),
            store.handle,
            buf,
...
function supplemental_attribute_summary(store::Store)
    json = _filter_probe(
        store,
        (buf, cap, out_len) -> ccall(
            (:ts_store_supplemental_attribute_summary, lib_path()),
            Int32,
            (Ptr{Cvoid}, Ptr{UInt8}, UInt64, Ref{UInt64}),
            store.handle,
            buf,
...
function add_parent_child_association!(store::Store, association::ParentChildAssociation)
    _check(
        ccall(
            (:ts_store_add_parent_child_association, lib_path()),
            Int32,
            (Ptr{Cvoid}, Int64, Cstring, Int64, Cstring),
            store.handle,
...
function add_parent_child_associations!(
    ...
    return _assoc_count_out(
        out -> ccall(
            (:ts_store_add_parent_child_associations, lib_path()),
            Int32,
            (Ptr{Cvoid}, Cstring, Ref{UInt64}),
            store.handle,
            payload,
...
function has_parent_child_association(
    ...
    return _assoc_bool_out(
        out -> ccall(
            (:ts_store_has_parent_child_association, lib_path()),
            Int32,
            (Ptr{Cvoid}, Cstring, Ref{Bool}),
            store.handle,
            filter_json,
...
function list_parent_child_associations(
    ...
    json = _filter_probe(
        store,
        (buf, cap, out_len) -> ccall(
            (:ts_store_list_parent_child_associations, lib_path()),
            Int32,
            (Ptr{Cvoid}, Cstring, Ptr{UInt8}, UInt64, Ref{UInt64}),
            store.handle,
            filter_json,
...
function _parent_child_ids(store::Store, filter_json, endpoint::Integer)
    json = _filter_probe(
        store,
        (buf, cap, out_len) -> ccall(
            (:ts_store_list_parent_child_ids, lib_path()),
            Int32,
...
function remove_parent_child_associations!(
    ...
    return _assoc_count_out(
        out -> ccall(
            (:ts_store_remove_parent_child_associations, lib_path()),
            Int32,
            (Ptr{Cvoid}, Cstring, Ref{UInt64}),
            store.handle,
            filter_json,
...
function replace_parent_child_component_id!(store::Store, old_id::Integer, new_id::Integer)
    return _assoc_count_out(
        out -> ccall(
            (:ts_store_replace_parent_child_component_id, lib_path()),
            Int32,
            (Ptr{Cvoid}, Int64, Int64, Ref{UInt64}),
            store.handle,
            Int64(old_id),
...
function count_parent_child_associations(
    ...
    _check(
        ccall(
            (:ts_store_count_parent_child_associations, lib_path()),
            Int32,
...
function get_forecast_parameters(
    ...
    code = ccall(
        (:ts_store_get_forecast_parameters, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Cstring,
...
function check_static_consistency(store::Store; resolution::Union{Nothing,Period}=nothing)
    ...
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_check_static_consistency, lib_path()),
        Int32,
        (Ptr{Cvoid}, Cstring, Ptr{UInt8}, UInt64, Ref{UInt64}),
        store.handle,
        fres,
...
function get_resolutions(store::Store; time_series_type::Union{Nothing,Integer}=nothing)
    ...
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_get_resolutions, lib_path()),
        Int32,
...
function get_intervals(store::Store; time_series_type::Union{Nothing,Integer}=nothing)
    ...
    out_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_get_intervals, lib_path()),
        Int32,
        (Ptr{Cvoid}, Bool, Int32, Ptr{UInt8}, UInt64, Ref{UInt64}),
        store.handle,
        has_type,
...
function read_only(store::Store)
    out = Ref{Bool}(false)
    code = ccall(
        (:ts_store_read_only, lib_path()), Int32, (Ptr{Cvoid}, Ref{Bool}), store.handle, out
    )
    _check(code)
    return out[]
end
...
function get_compression(store::Store)
    ...
    code = ccall(
        (:ts_store_get_compression, lib_path()),
        Int32,
...
function verify_integrity(store::Store)
    out = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_verify, lib_path()), Int32, (Ptr{Cvoid}, Ref{UInt64}), store.handle, out
    )
    _check(code)
    return Int(out[])
end
...
function compact!(store::Store)
    code = ccall((:ts_store_compact, lib_path()), Int32, (Ptr{Cvoid},), store.handle)
    _check(code)
    return nothing
end
...
function flush!(store::Store)
    code = ccall((:ts_store_flush, lib_path()), Int32, (Ptr{Cvoid},), store.handle)
    _check(code)
    return nothing
end
...
function persist!(store::Store, path::AbstractString)
    code = ccall(
        (:ts_store_persist, lib_path()), Int32, (Ptr{Cvoid}, Cstring), store.handle, path
    )
    _check(code)
    return nothing
end
...
function replace_owner!(
    ...
    out = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_replace_owner, lib_path()),
        Int32,
...
function _add_dense_forecast!(
    ...
    out_key = Ref{Ptr{Cvoid}}(C_NULL)
    code = ccall(
        (:ts_store_add_forecast, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function transform_single_time_series!(
    ...
    out_count = Ref{UInt64}(0)
    code = ccall(
        (:ts_store_transform_single_time_series, lib_path()),
        Int32,
...
function remove_typed!(
    ...
    features_json = _features_arg(features)
    code = ccall(
        (:ts_store_remove_typed, lib_path()),
        Int32,
...
function copy_time_series!(
    ...
    code = ccall(
        (:ts_store_copy_time_series, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function add_time_series!(
    ...
    out_key = Ref{Ptr{Cvoid}}(C_NULL)
    code = ccall(
        (:ts_store_add_probabilistic, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function add_time_series!(
    ...
    bytes = _row_major_bytes(ts.data)
    code = ccall(
        (:ts_batch_add_non_sequential, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function _batch_add_dense_forecast!(
    ...
    bytes = _row_major_bytes(data)
    code = ccall(
        (:ts_batch_add_forecast, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function add_time_series!(
    ...
    bytes = _row_major_bytes(ts.data)
    code = ccall(
        (:ts_batch_add_probabilistic, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Int64,
...
function _get_forecast_raw(
    ...
    code = ccall(
        (:ts_store_get_forecast, lib_path()),
        Int32,
        (
            Ptr{Cvoid},   # handle
            Int64,        # owner_id
...
function _get_forecast_raw(
    ...
    code = ccall(
        (:ts_store_get_forecast_by_key, lib_path()),
        Int32,
        (
            Ptr{Cvoid},   # handle
            Ptr{Cvoid},   # key
...
function get_time_series(
    ...
    r = _get_forecast_raw(
        ...
        owner_category,
        name,
        TS_TYPE_ABSTRACT_DETERMINISTIC;
        resolution=resolution,
        interval=interval,
        features=features,
...
function _static_group_layout(handle::Ptr{Cvoid}, gi::Integer)
    ...
    out_shape_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_static_reader_group_info, lib_path()),
        Int32,
    ...
    for col in 0:(Int(out_ncols[]) - 1)
        out_key = Ref{Ptr{Cvoid}}(C_NULL)
        code = ccall(
            (:ts_static_reader_group_key, lib_path()),
            Int32,
...
function build_static_reader(
    ...
    out = Ref{Ptr{Cvoid}}(C_NULL)
    code = ccall(
        (:ts_store_build_static_reader, lib_path()),
        Int32,
    ...
    _check(
        ccall(
            (:ts_static_reader_num_groups, lib_path()),
            Int32,
...
function static_grid(reader::StaticReader)
    ...
    _check(
        ccall(
            (:ts_static_reader_grid, lib_path()),
            Int32,
...
function static_read!(reader::StaticReader, t::DateTime)
    _check(
        ccall(
            (:ts_static_reader_read, lib_path()),
            Int32,
...
function static_values(reader::StaticReader, group_index::Integer)
    ...
    _check(
        ccall(
            (:ts_static_reader_group_values, lib_path()),
            Int32,
...
function _forecast_entry_layout(handle::Ptr{Cvoid}, ei::Integer)
    ...
    out_shape_len = Ref{UInt64}(0)
    code = ccall(
        (:ts_forecast_reader_entry_info, lib_path()),
        Int32,
    ...
    _check(
        ccall(
            (:ts_forecast_reader_entry_key, lib_path()),
            Int32,
    ...
    _check(
        ccall(
            (:ts_forecast_reader_entry_slot, lib_path()),
            Int32,
...
function build_forecast_reader(
    ...
    out = Ref{Ptr{Cvoid}}(C_NULL)
    code = ccall(
        (:ts_store_build_forecast_reader, lib_path()),
        Int32,
        (
            Ptr{Cvoid},
            Bool,
    ...
    _check(
        ccall(
            (:ts_forecast_reader_num_entries, lib_path()),
            Int32,
            (Ptr{Cvoid}, Ref{UInt64}),
            handle,
            out_n,
...
function forecast_timeline(reader::ForecastReader)
    ...
    _check(
        ccall(
            (:ts_forecast_reader_timeline, lib_path()),
            Int32,
...
function forecast_num_slots(reader::ForecastReader)
    ...
    _check(
        ccall(
            (:ts_forecast_reader_num_slots, lib_path()),
            Int32,
            (Ptr{Cvoid}, Ref{UInt64}),
            reader.handle,
            out_n,
...
function forecast_read!(reader::ForecastReader, t::DateTime)
    _check(
        ccall(
            (:ts_forecast_reader_read, lib_path()),
            Int32,
            (Ptr{Cvoid}, Ptr{Cvoid}, Int64),
            reader.handle,
            reader.store.handle,
...
function forecast_values(reader::ForecastReader, entry_index::Integer)
    ...
    _check(
        ccall(
            (:ts_forecast_reader_entry_values, lib_path()),
            Int32,
            (Ptr{Cvoid}, UInt64, Ref{Ptr{UInt8}}, Ref{UInt64}),
            reader.handle,
