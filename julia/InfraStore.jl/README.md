...
The package wraps the `libinfrastore_ffi` C ABI, distributed as `InfraStore_jll` and built against
`NetCDF_jll` / `HDF5_jll` so a Julia process has exactly one `libhdf5`.
