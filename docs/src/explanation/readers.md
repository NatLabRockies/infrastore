...
regardless, and when a chunk carries several rows they are the next ones the sweep asks for.
Scattered access to a single timestamp is the case where extra rows are decompressed and discarded.
