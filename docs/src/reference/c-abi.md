...
`castore-ffi` compiles a `cdylib` (`libcastore_ffi.{dylib,so,dll}`) exposing a C-compatible API over
`Store`. The header `crates/castore-ffi/include/castore.h` is generated by cbindgen — do not
hand-edit it. The [Julia binding](./julia-api.md) is the primary consumer.

## Conventions

- **Return codes.** Every function returns `int32_t`. `CASTORE_OK` (0) is success; any other value
  is an error. Retrieve the human-readable detail with `castore_last_error_message`.
- **Opaque handles.** `CastoreStore` and `CastoreKey` are incomplete struct types; you only ever
  hold pointers. Free them with `castore_store_free` and `castore_key_free`.
...
- **Caller-owned buffers.** Returned arrays come back through an out-pointer + length and must be
  freed: `f64` percentile buffers (`double **`) with `castore_buffer_free_f64`, raw element-byte
  buffers (`uint8_t **`) with `castore_buffer_free_u8`, timestamp and shape buffers (`int64_t **`)
  with `castore_buffer_free_i64`, and the `u64` dims buffer from `castore_store_get_forecast`
  (`uint64_t **`) with `castore_buffer_free_u64`.
- **Typed arrays.** Add functions take the element `dtype` code
  ...
  the dtype code and a raw byte buffer for the caller to decode; `castore_store_get_single` follows
  the same dtype-generic convention, returning the dtype, shape, and raw element bytes.
...
- **Timestamps** are `int64_t` Unix milliseconds. **Resolutions/horizons/intervals** are ISO-8601
  ...
  output they are owned `char *` strings — free each with `castore_string_free`.

## Status Codes

| Macro                               | Value | Meaning                                                                                                                                               |
| ----------------------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CASTORE_OK`                        | 0     | Success                                                                                                                                               |
| `CASTORE_ERR_NULL_POINTER`          | 1     | A required pointer was `NULL`                                                                                                                         |
| `CASTORE_ERR_INVALID_UTF8`          | 2     | A string argument was not UTF-8                                                                                                                       |
| `CASTORE_ERR_INVALID_PARAMETER`     | 3     | A bad argument value                                                                                                                                  |
| `CASTORE_ERR_NOT_FOUND`             | 4     | No matching series / array                                                                                                                            |
| `CASTORE_ERR_DUPLICATE`             | 5     | Key already exists                                                                                                                                    |
| `CASTORE_ERR_INTEGRITY`             | 6     | On-disk inconsistency                                                                                                                                 |
| `CASTORE_ERR_READ_ONLY`             | 7     | Write on a read-only store                                                                                                                            |
| `CASTORE_ERR_IO`                    | 8     | I/O failure                                                                                                                                           |
| `CASTORE_ERR_INCOMPATIBLE_FORMAT`   | 9     | The store on disk was written in a different, incompatible on-disk format than this build reads. There is no in-place upgrade.                        |
| `CASTORE_ERR_DUPLICATE_ASSOCIATION` | 10    | An attachment or parent/child edge with the same identity already exists. Distinct from `CASTORE_ERR_DUPLICATE`, which is about time-series identity. |
| `CASTORE_ERR_INTERNAL`              | 99    | Unexpected internal error                                                                                                                             |
...
int32_t castore_store_create_with_compression(const char *path, bool in_memory, uint8_t compression_kind,
                                         uint8_t deflate_level, bool shuffle, struct CastoreStore **out);
...
int32_t castore_store_get_single(const struct CastoreStore *handle, const struct CastoreKey *key,
...
int32_t castore_store_remove_bulk(struct CastoreStore *handle,
                             const struct CastoreKey *const *keys, uint64_t len,
                             uint64_t *out_removed);
int32_t castore_store_has(const struct CastoreStore *handle, const struct CastoreKey *key, bool *out_present);
...
`castore_store_add_non_sequential` takes an explicit `int64_t` Unix-millisecond timestamp array
alongside the typed data buffer. `castore_store_get_non_sequential` returns owned timestamp, shape,
...
int32_t castore_store_get_non_sequential(const struct CastoreStore *handle, const struct CastoreKey *key,
...
int32_t castore_store_get_array_by_hash(const struct CastoreStore *handle, const uint8_t *data_hash,
                                   int32_t *out_dtype,
                                   uint8_t **out_data, uint64_t *out_byte_len); /* castore_buffer_free_u8 */
...
int32_t castore_store_count_array_references(const struct CastoreStore *handle, const uint8_t *data_hash,
                                        uint64_t *out_sts, uint64_t *out_dst);

/* Build a key handle from attributes for any ts_type, so an attribute-addressed
   caller can reuse the key-based readers (castore_store_get_single,
   castore_store_get_non_sequential, castore_store_get_forecast_by_key). A NULL resolution
   or interval means unset. The returned key is owned; free it with castore_key_free. */
int32_t castore_make_key_from_attrs(int64_t owner_id, int32_t owner_category, const char *name,
                               int32_t ts_type,
...
/* List every key for owner_id (one per association, including derived
   DeterministicSingleTimeSeries rows). Ownership is two-tiered: free each CastoreKey
   with castore_key_free, then free the array with castore_keys_buffer_free. An owner with
   no series yields *out_keys = NULL and *out_len = 0. */
int32_t castore_store_get_time_series_keys(const struct CastoreStore *handle,
                                      int64_t owner_id, int32_t owner_category,
                                      struct CastoreKey ***out_keys, uint64_t *out_len);
...
/* Inspect an opaque key: type code, resolution (an owned ISO-8601 string, NULL
   when unset — free with castore_string_free), owner id, owner category (0 = Component,
...
int32_t castore_key_attributes(const struct CastoreKey *key,
                          int32_t *out_type, char **out_resolution,  /* ISO-8601; castore_string_free */
...
/* Read an association's name by key, resolved through the stored metadata (the
   ...
   probe-then-fetch convention (like castore_key_attributes): call with name_buf = NULL,
   name_cap = 0 to learn *out_name_len, then again with a name_len+1-byte buffer. */
int32_t castore_store_get_association(const struct CastoreStore *handle, const struct CastoreKey *key,
                                 char *name_buf, uint64_t name_cap, uint64_t *out_name_len);
```

`castore_store_get_metadata` + `castore_store_get_array_by_hash` is the read path used by bindings
that maintain their own key objects (such as an InfrastructureSystems.jl-side store).
`castore_make_key_from_attrs` bridges the two addressing styles: it materializes a `CastoreKey` from
attributes that the key-based read functions accept directly. `castore_store_get_time_series_keys`
enumerates an owner's keys (the only way to obtain a key for a transform-derived
`DeterministicSingleTimeSeries`), and `castore_key_attributes` reads back an opaque key's type,
name, features, and addressing so the caller can pick the matching key-based reader.
...
sentinel, `CASTORE_TYPE_ABSTRACT_DETERMINISTIC` (`100`), described below. Forecast values are
...
`castore_store_add_forecast` — it is derived via `castore_store_transform_single_time_series`.

The C ABI keeps these per-type add functions as low-level transport (the higher-level bindings layer
the generic `add_time_series` over them). `castore_store_add_forecast` accepts only `ts_type`
`2 =
Deterministic` or `5 = Scenarios`; `castore_store_add_probabilistic` adds the percentile vector
for `Probabilistic`. `DeterministicSingleTimeSeries` (`3`) is **not** addable through
`castore_store_add_forecast` — it errors and directs you to
`castore_store_transform_single_time_series`, which derives a `DeterministicSingleTimeSeries` from
every stored `SingleTimeSeries` (sharing the backing array) and writes the number transformed to
`*out_count`:
...
`castore_store_get_forecast` is the forecast read function: it resolves a forecast by attributes and
...
- A concrete code (`2 = Deterministic`, `3 = DeterministicSingleTimeSeries`, `4 = Probabilistic`,
  ...
  non-forecast codes `0` and `1` are rejected with `CASTORE_ERR_INVALID_PARAMETER`.
- `CASTORE_TYPE_ABSTRACT_DETERMINISTIC` (`100`) is a request-only sentinel — never a stored type —
  ...
  returns `CASTORE_ERR_INVALID_PARAMETER`; a genuine miss returns the usual not-found error.
...
caller owns the returned buffers: free `*out_data` with `castore_buffer_free_u8`, `*out_dims` with
`castore_buffer_free_u64`, `*out_percentiles` (non-NULL only for `Probabilistic`) with
`castore_buffer_free_f64`, and each of the `*out_resolution` / `*out_horizon` / `*out_interval`
ISO-8601 strings with `castore_string_free`.

```c
int32_t castore_store_get_forecast(const struct CastoreStore *handle,
                              int64_t owner_id, int32_t owner_category,
                              const char *name,
                              int32_t ts_type,  /* 2..5, or CASTORE_TYPE_ABSTRACT_DETERMINISTIC (100) */
                              ...
                              uint64_t *out_ndims, uint64_t **out_dims,  /* dims: castore_buffer_free_u64 */
                              int32_t *out_dtype,
                              uint8_t **out_data, uint64_t *out_data_byte_len, /* castore_buffer_free_u8 */
                              double **out_percentiles, uint64_t *out_percentiles_len, /* castore_buffer_free_f64 */
                              int32_t *out_matched_type);  /* concrete matched TimeSeriesType */
```

`castore_store_get_forecast_by_key` is the key-based counterpart: it takes a `CastoreKey` handle
(the type comes from the key) instead of the
...
`castore_store_get_forecast` for a family request.

```c
int32_t castore_store_get_forecast_by_key(const struct CastoreStore *handle, const struct CastoreKey *key,
                                     bool time_range_present,
                                     int64_t time_range_start_ms, int64_t time_range_end_ms,
                                     int64_t *out_initial_ts_unix_ms,
                                     char **out_resolution, char **out_horizon, char **out_interval,  /* ISO-8601; castore_string_free */
                                     uint64_t *out_count, uint64_t *out_scenario_count,
                                     uint64_t *out_ndims, uint64_t **out_dims, /* castore_buffer_free_u64 */
                                     int32_t *out_dtype,
                                     uint8_t **out_data, uint64_t *out_data_byte_len, /* castore_buffer_free_u8 */
                                     double **out_percentiles, uint64_t *out_percentiles_len, /* castore_buffer_free_f64 */
...
array. `castore_store_get_probabilistic_metadata` additionally returns the percentile vector (free
it with `castore_buffer_free_f64`):
...
int32_t castore_store_get_probabilistic_metadata(const struct CastoreStore *handle,
                                            ...
                                            char **out_interval,  /* ISO-8601; castore_string_free */
                                            uint64_t *out_count,
...
`castore_store_copy_time_series` copies one association onto another owner (optionally under a new
name). Arrays are content-addressed, so only a new association row is written — no array data is
...
`castore_store_remove_typed`; a `NULL` `new_name` keeps the source name.
...
The per-timestamp read path is exposed as two opaque reader handles — `CastoreStaticReaderHandle`
for `SingleTimeSeries` and `CastoreForecastReaderHandle` for forecasts. A reader is built once over
a filter (the same `has_owner` / `owner_id` / `has_owner_category` / `owner_category` / `name` /
`resolution` / `features_json` convention as the attribute-based access, with a forecast reader also
taking a `ts_type`), then driven per timestamp. The lifecycle is: **build → read the layout once →
`*_read` in a loop → fetch values per group/entry → free**. Each reader pins one resolution and owns
reusable buffers that each read overwrites in place.

Ownership rules: the `*_grid` / `*_timeline` resolution/interval out-strings (`char **`) are owned —
free each with `castore_string_free`. Keys from `*_group_key` / `*_entry_key` are owned
`CastoreKey *` — free with `castore_key_free`. The `*_values` buffers (`const uint8_t **`) are
...
column `j` is the key from `castore_static_reader_group_key(reader, group_idx, j, …)`.
...
int32_t castore_static_reader_grid(const struct CastoreStaticReaderHandle *reader,
                              int64_t *out_initial_ms, char **out_resolution,  /* free with castore_string_free */
                              uint64_t *out_length);
int32_t castore_static_reader_num_groups(const struct CastoreStaticReaderHandle *reader, uint64_t *out_n);
int32_t castore_static_reader_group_info(const struct CastoreStaticReaderHandle *reader, uint64_t group_idx,
...
int32_t castore_static_reader_group_key(const struct CastoreStaticReaderHandle *reader,
                                   ...
                                   struct CastoreKey **out_key);  /* free with castore_key_free */
int32_t castore_static_reader_read(struct CastoreStaticReaderHandle *reader,
                              const struct CastoreStore *store, int64_t at_unix_ms);
int32_t castore_static_reader_group_values(const struct CastoreStaticReaderHandle *reader, uint64_t group_idx,
...
with `castore_store_get_forecast`, where `2` matches only a stored `Deterministic` and the family
request must be spelled `CASTORE_TYPE_ABSTRACT_DETERMINISTIC`; the reader build takes no such
...
int32_t castore_forecast_reader_timeline(const struct CastoreForecastReaderHandle *reader,
                                    ...
                                    char **out_resolution, char **out_interval,  /* free each with castore_string_free */
                                    uint64_t *out_count);
int32_t castore_forecast_reader_num_entries(const struct CastoreForecastReaderHandle *reader, uint64_t *out_n);
int32_t castore_forecast_reader_num_slots(const struct CastoreForecastReaderHandle *reader, uint64_t *out_n);
...
int32_t castore_forecast_reader_entry_info(const struct CastoreForecastReaderHandle *reader, uint64_t entry_idx,
                                      int32_t *out_dtype,
                                      int64_t *shape_buf, uint64_t shape_cap, uint64_t *out_shape_len);
int32_t castore_forecast_reader_entry_key(const struct CastoreForecastReaderHandle *reader,
                                     uint64_t entry_idx, struct CastoreKey **out_key);  /* free with castore_key_free */
int32_t castore_forecast_reader_read(struct CastoreForecastReaderHandle *reader,
                                const struct CastoreStore *store, int64_t at_unix_ms);
int32_t castore_forecast_reader_entry_values(const struct CastoreForecastReaderHandle *reader, uint64_t entry_idx,
                                        const uint8_t **out_ptr,  /* borrowed; valid until next read/free */
                                        uint64_t *out_byte_len);
...
`SingleTimeSeries`) collapse to a single _window slot_. `castore_forecast_reader_read` performs one
backend read per slot, not per entry, so a forecast shared by N owners is read once per timestamp.
`castore_forecast_reader_num_slots` is that physical read count, and
`castore_forecast_reader_entry_slot` gives the 0-based slot backing each entry (entries that share
data report the same slot) — group entries by slot to also decode each unique window only once.

## Bulk Adds

A batch accumulates add requests client-side (no store I/O); `castore_store_add_batch` commits them
all in **one** metadata transaction, which is much faster than per-item adds when ingesting many
series. It is also the fast NetCDF write path: same-shaped `SingleTimeSeries` are packed into
batch-sized datasets so the timestamp-major chunks are filled whole rather than a column at a time.
The `castore_batch_add_*` functions take the same arguments as their `castore_store_add_*`
...
key-handle array: free each key with `castore_key_free`, then the buffer with
`castore_keys_buffer_free` (same contract as `castore_store_get_time_series_keys`). The batch handle
itself is reusable after submit and must eventually be released with `castore_batch_free`.

```c
struct CastoreBatch *castore_batch_new(void);
void            castore_batch_free(struct CastoreBatch *batch);

int32_t castore_batch_add_single(struct CastoreBatch *batch, /* castore_store_add_single args sans handle/out_key */ ...);
int32_t castore_batch_add_non_sequential(struct CastoreBatch *batch, ...);
int32_t castore_batch_add_forecast(struct CastoreBatch *batch, ...);       /* 2=Deterministic, 5=Scenarios */
int32_t castore_batch_add_probabilistic(struct CastoreBatch *batch, ...);

int32_t castore_store_add_batch(struct CastoreStore *handle, struct CastoreBatch *batch,
                           struct CastoreKey ***out_keys, uint64_t *out_len);
```

## Bulk Reads

`castore_store_bulk_read_single` reads many full `SingleTimeSeries` in one call, reading each packed
...
otherwise the call fails with `CASTORE_ERR_INVALID_PARAMETER`. The results are held in a
`CastoreBulkReadHandle` (input order preserved) and read out element-by-element with
`castore_bulk_result_get_single`, whose out-parameters match `castore_store_get_single` — the caller
owns the returned resolution string and the shape/data buffers and frees them with
`castore_string_free`, `castore_buffer_free_i64`, and `castore_buffer_free_u8`. The handle is not
consumed by a read (elements may be read more than once) and must be released with
`castore_bulk_result_free`.

```c
int32_t castore_store_bulk_read_single(const struct CastoreStore *handle,
                                  const struct CastoreKey *const *keys, uint64_t n,
                                  struct CastoreBulkReadHandle **out_result);
int64_t castore_bulk_result_len(const struct CastoreBulkReadHandle *result);   /* -1 if null */
int32_t castore_bulk_result_get_single(const struct CastoreBulkReadHandle *result, uint64_t index,
                                  /* same out-params as castore_store_get_single */ ...);
...
```c
int32_t castore_store_counts(const struct CastoreStore *handle, int64_t *out_components_with_time_series,
...
int32_t castore_store_counts_detailed(const struct CastoreStore *handle, int64_t *out_components,
...
int32_t castore_store_list_owner_ids(const struct CastoreStore *handle, int32_t owner_category,
...
/* out_present = false when no matching forecast. The out_horizon/out_interval/
   out_resolution ISO-8601 strings are NULL when absent (free with castore_string_free);
...
int32_t castore_store_get_compression(const struct CastoreStore *handle, uint8_t *out_kind,
...
int32_t castore_store_clear(struct CastoreStore *handle, bool has_owner, int64_t owner_id,
...
int32_t castore_store_list_keys(const struct CastoreStore *handle,
                           ...
                           char *buf, uint64_t cap, uint64_t *out_len);
/* Like castore_store_list_keys, but each row is annotated with the hex content hash of
   ...
   convention as castore_store_list_keys. */
...
`CASTORE_ERR_INVALID_PARAMETER`. Every remove function reports its count through `out_removed` and
treats removing nothing as success, not an error.

The list functions follow the same probe-then-fetch convention as `castore_store_list_keys`: call
...
/* Attach supplemental attribute (attribute_id, attribute_type) to component
   (component_id, component_type). CASTORE_ERR_DUPLICATE_ASSOCIATION if that component
...
/* Attach many in one all-or-nothing transaction, from a JSON array of objects with
   ...
   of the round trip whose export is castore_store_list_supplemental_attribute_associations
...
/* Move every attachment from component old_id to new_id; *out_updated (when
   non-NULL) receives the rows changed. CASTORE_ERR_DUPLICATE_ASSOCIATION if new_id
...
uint64_t len = 0;
castore_store_list_supplemental_attribute_ids(store, filter, NULL, 0, &len);
char *json = malloc(len + 1);
castore_store_list_supplemental_attribute_ids(store, filter, json, len + 1, &len); /* e.g. "[100]" */
free(json);
```
...
/* Record a directed edge from component (parent_id, parent_type) to component
   (child_id, child_type). CASTORE_ERR_DUPLICATE_ASSOCIATION if that ordered pair is
...
/* Rewrite component old_id to new_id on BOTH ends of every edge; *out_updated
   (when non-NULL) receives the rows changed. CASTORE_ERR_DUPLICATE_ASSOCIATION if the
...
uint64_t len = 0;
castore_store_list_parent_child_ids(store, filter, 1 /* children */, NULL, 0, &len);
char *json = malloc(len + 1);
castore_store_list_parent_child_ids(store, filter, 1, json, len + 1, &len);   /* e.g. "[7]" */
free(json);
```
...
length excluding the NUL. If `buf_len` is too small, the message is truncated but `CASTORE_OK` is
...
# Library at: target/release/libcastore_ffi.{dylib,so,dll}
```

Point consumers at it via the `CASTORE_LIB` environment variable (see the
...
directive string (e.g. `"debug"` or `"castore_core=debug"`). Pass `NULL` to read the `RUST_LOG`
...
`CASTORE_OK` on success or `CASTORE_ERR_INVALID_UTF8` if `filter` is not valid UTF-8.
