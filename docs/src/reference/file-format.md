...
(`0.18.0` made `time_series_associations.id` an `AUTOINCREMENT` primary key and surfaces it to
consumers as `association_id`, exposed by `get_time_series_metadata_by_association_id`. The row owns
its id, so a rename or an owner reassignment leaves it alone and a consumer may hold it as a durable
