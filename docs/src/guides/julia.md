...
This guide covers building on `TimeSeries.jl`, the Julia package that wraps the
...
`TimeSeries.jl` resolves the native library from the `TIME_SERIES_STORE_LIB` environment variable.
Build the cdylib and point at it before `using TimeSeries`:
...
Exported names include `TimeSeriesStore`, `SingleTimeSeries`, `OwnerCategory` (`Component`,
`SupplementalAttribute`), and the `add_time_series!` / `get_time_series` / `get_metadata` family.
...
store = TimeSeriesStore(in_memory=false, path="system.nc")
...
Beyond key handles, `TimeSeries.jl` can resolve a series directly from its attributes — convenient
when a caller keeps its own identifiers (as an IS.jl-side store does):
...
values = get_array_by_hash(store, meta.data_hash)     # Vector{Float64}
