...
- **Per-step time** for the read benchmark scales linearly with `--count`. If the cost per step is
  ...
  ([file format](../reference/file-format.md#packed-datasets)) — so reading one timestamp across
  many series is a chunk read per dataset, while reading one full series touches every chunk band
  (the slow direction, expected for exploration/plotting rather than hot loops).
