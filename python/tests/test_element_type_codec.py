...
`conformance/element_type_vectors.json` is generated by `infrastore-core`'s
`tests/element_type_conformance.rs` and read by every binding's codec tests, so
all implementations are held to one definition of the encodings rather than to
each other.
"""
