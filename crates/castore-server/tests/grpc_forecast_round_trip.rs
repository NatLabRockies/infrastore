...
async fn resolve_forecast_key_and_bulk_over_grpc() {
    let mut store = castore_core::create_store(None, true).unwrap();
    ...
    let key = client
        .resolve_forecast_key(
            ...
            None,
            Features::new(),
            castore_core::RequestedType::AbstractDeterministic,
        )
        .await
        .unwrap();
    assert_eq!(
        key.time_series_type(),
        castore_core::TimeSeriesType::Deterministic
    );

    // Full-key snapshot carries horizon + count for a forecast key.
    match &key {
        castore_core::TimeSeriesKey::Forecast(f) => {
            assert_eq!(f.count, 6);
