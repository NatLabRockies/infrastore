...
async fn missing_key_returns_not_found() {
    ...
    let bogus_key = castore_core::KeyIdentity {
        owner_id: 999,
    ...
    assert!(matches!(err, castore_core::TimeSeriesError::NotFound));
}
...
async fn additive_read_rpcs() {
    ...
    assert_eq!(keys.len(), 1);
    match &keys[0] {
        castore_core::TimeSeriesKey::Single(s) => assert_eq!(s.length, 24),
