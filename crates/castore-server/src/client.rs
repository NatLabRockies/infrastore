...
//!
//! [`RemoteClient`] mirrors the read methods of [`castore_core::Store`]
...
fn decode_full_key(k: castore_proto::pb::TimeSeriesKey) -> CoreResult<TimeSeriesKey> {
...
fn build_list_req(
    ...
    features: Option<&castore_core::Features>,
) -> ListReq {
    ListReq {
        owner_id,
...
impl RemoteClient {
    ...
    pub async fn get_counts(&self) -> CoreResult<castore_core::TimeSeriesCounts> {
        let mut inner = self.inner.lock().await;
        let resp = inner
            ...
            .map_err(Self::map_status)?
            .into_inner();
        Ok(castore_core::TimeSeriesCounts {
    ...
    pub async fn get_forecast_parameters(
        ...
        resolution: Option<Period>,
        interval: Option<Period>,
    ) -> CoreResult<castore_core::ForecastParameters> {
        let mut inner = self.inner.lock().await;
        let resp = inner
        ...
        Ok(castore_core::ForecastParameters {
    ...
    pub async fn verify_integrity(&self) -> CoreResult<castore_core::storage::IntegrityReport> {
        let mut inner = self.inner.lock().await;
        let resp = inner
            ...
            .map_err(Self::map_status)?
            .into_inner();
        Ok(castore_core::storage::IntegrityReport {
            errors: resp.errors,
        })
    }
    ...
    pub async fn resolve_forecast_key(
        ...
        resolution: Option<Period>,
        interval: Option<Period>,
        features: castore_core::Features,
