...
fn read_only_blocks_writes() {
    ...
    // Reopen read-only.
    let mut ro = castore_core::open_store(path.as_path() as &Path, true).unwrap();
    let s = series(2024, 12, 1.0);
    let err = ro
        .add_time_series(
            2,
...
fn only_key(store: &castore_core::Store, owner: i64) -> KeyIdentity {
    let keys = store
...
fn static_consistency_is_checked_per_resolution() {
    let initial = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let mut store = create_store(None, true).unwrap();
    let add = |store: &mut castore_core::Store, owner: i64, res, len: usize| {
