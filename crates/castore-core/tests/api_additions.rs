...
fn add_sts(store: &mut castore_core::Store, owner: i64, name: &str, base: f64) -> TimeSeriesKey {
    store
        .add(AddRequest::new(
            owner,
            "Generator",
...
fn replace_owner_is_rejected_on_a_read_only_store() {
    ...
    {
        let mut store = castore_core::create_store(Some(path.as_path()), false).unwrap();
        add_sts(&mut store, 1, "load", 10.0);
        store.flush().unwrap();
    }
    let mut store = castore_core::open_store(path.as_path(), true).unwrap();
    assert!(matches!(
...
fn read_only_and_path_accessors_report_each_store_state() {
    ...
    let rw = castore_core::open_store(path.as_path(), false).unwrap();
    assert!(!rw.read_only());
    assert_eq!(rw.netcdf_path(), Some(path.as_path()));
    drop(rw);

    let ro = castore_core::open_store(path.as_path(), true).unwrap();
    assert!(ro.read_only());
...
fn a_read_only_store_rejects_every_write_entry_point() {
    ...
    let is_ro = |r: castore_core::Result<()>| matches!(r, Err(TimeSeriesError::ReadOnlyStore));
