...
/// this record as `FilterRecord` ships as a version-pinned artifact built from
/// the same tree. A size or version prefix would relax that, and is worth
/// adding the day a caller ships separately from the library; there is no such
/// caller today.
#[repr(C)]
...
impl Default for InfraStoreFilter {
    ...
    fn default() -> Self {
        Self {
            has_owner_id: false,
            owner_id: 0,
            has_owner_category: false,
            owner_category: 0,
            has_time_series_type: false,
            time_series_type: 0,
            name: ptr::null(),
            name_glob: ptr::null(),
            resolution: ptr::null(),
            interval: ptr::null(),
            features_json: ptr::null(),
            features_exact: false,
            component_field: ptr::null(),
            has_zoneless: false,
            zoneless: false,
            has_initial_timestamp: false,
...
pub unsafe extern "C" fn infrastore_store_build_forecast_reader(
    ...
    let mut filter = match unsafe { build_filter(filter) } {
        Ok(f) => f,
        Err(c) => return c,
    };
