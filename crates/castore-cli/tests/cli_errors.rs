//! Failure paths and untested subcommands for the `cas` binary.
//!
...
//! `CASTORE_STORE` environment fallback.
...
const BIN: &str = env!("CARGO_BIN_EXE_cas");

/// Run `cas --store <store> <args...>`, asserting success; returns stdout.
...
/// Run `cas`, asserting a nonzero exit without requiring the `Error: ` prefix.
...
/// `cas add` for a fixture descriptor, expecting failure; returns stderr.
...
/// `cas add` for a fixture descriptor, expecting success.
...
// ---------------------------------------------------------------------------
// Store selection: CASTORE_STORE, precedence, and the missing-store error
// ---------------------------------------------------------------------------

#[test]
fn the_castore_store_env_var_is_used_when_no_flag_is_given() {
    let dir = tempfile::tempdir().unwrap();
    let store = dir.path().join("store.nc");
    seed(dir.path(), &store);

    let output = Command::new(BIN)
        .env("CASTORE_STORE", &store)
        .args(["-f", "csv", "get", "--owner-id", "42", "--name", "load"])
        .output()
        .unwrap();
    assert!(
        output.status.success(),
        "CASTORE_STORE was not honored:\nstderr: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    assert_eq!(
...
fn the_store_flag_beats_the_env_var() {
    ...
    assert_eq!(
        data_lines(&String::from_utf8_lossy(&output.stdout)),
        vec!["10", "11", "12", "13"],
        "--store must win over CASTORE_STORE"
    );
}
...
fn verify_of_a_store_whose_catalog_was_corrupted_still_exits_zero() {
    ...
    // to `cas verify` even though every read of that key now fails. Pinned here
    ...
    assert_eq!(
        exit_code(&store, &["verify"]),
        0,
        "PIN: a catalog-side corruption is invisible to `cas verify`"
    );
