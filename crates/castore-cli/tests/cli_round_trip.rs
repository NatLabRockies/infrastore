//! End-to-end round-trip tests: drive the `cas` binary to add series from a
...
/// Run `cas --store <store> <args...>`, asserting success, returning stdout.
...
fn completions_generate() {
    ...
    assert!(text.contains("_cas"), "zsh completion body: {text}");
}
