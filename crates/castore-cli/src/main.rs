...
#[command(
    name = "cas",
    version,
    about = "Load and inspect a castore store on disk",
    styles = HELP_STYLES
)]
struct Cli {
    /// Path to the NetCDF store file (.nc). The SQLite catalog is derived
    /// automatically. Falls back to the CASTORE_STORE environment variable.
    #[arg(long, global = true, env = "CASTORE_STORE")]
...
enum Commands {
    ...
    /// Generate shell completions to stdout (e.g. `cas completions zsh`).
    Completions {
...
fn run(cli: &Cli) -> Result<(), String> {
    match &cli.command {
        ...
        Commands::Completions { shell } => {
            ...
            clap_complete::generate(*shell, &mut Cli::command(), "cas", &mut std::io::stdout());
            Ok(())
        }
