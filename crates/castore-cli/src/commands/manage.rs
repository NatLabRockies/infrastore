...
pub fn clear(
    ...
    if dry_run {
        let store = store_access::open_readonly(store_path)?;
        let mut filter = castore_core::ListFilter::new();
...
pub fn replace_owner(
    ...
    if dry_run {
        let store = store_access::open_readonly(store_path)?;
        let filter = castore_core::ListFilter::new()
            .owner_id(old)
...
pub fn template(ts_type: &str) -> Result<(), String> {
    ...
    use castore_core::TimeSeriesType::*;
    let body = match kind {
        ...
        DeterministicSingleTimeSeries => {
            return Err(
                "DeterministicSingleTimeSeries is derived via `cas transform`, not a descriptor"
                    .to_string(),
            );
        }
    };
