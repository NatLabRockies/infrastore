...
//! the little-endian bytes, and the values a correct decoder produces. The
//! tests below round-trip them; the same corpus is exported to
//! `conformance/element_type_vectors.json` at the repo root, which the Python
//! and Julia codec tests read so every implementation is held to one definition
//! of the encoding rather than to each other.
//!
...
//!
//! This lives under `tests/` rather than in the library: the corpus is a test
//! fixture, and shipping it as a `pub mod` put a fixture generator in the
//! public API. Everything it needs is public, so nothing is lost by the move.
...
use infrastore_core::{
    DecodedValues, Dtype, ElementType, LinearFunction, QuadraticFunction, StepFunction,
    TimeSeriesType, TypedArray, XyPoint,
};
...
fn vector(
    ...
    let array = infrastore_core::encode(&decoded, leading_dims_shape)
...
fn vectors_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../conformance/element_type_vectors.json")
}

fn rendered() -> String {
    let mut json = serde_json::to_string_pretty(&serde_json::json!({
        "comment": "Generated by infrastore-core's tests/element_type_conformance.rs. \
...
fn every_vector_round_trips() {
    for v in vectors() {
        ...
        let decoded = infrastore_core::decode(&array, element_type, v.leading_dims)
        ...
        let re_encoded = infrastore_core::encode(&decoded, leading)
...
/// The exported JSON is what the Python and Julia codec tests
    ...
    let have = std::fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!(
            "{} is missing ({e}); regenerate with UPDATE_CONFORMANCE_VECTORS=1 \
             cargo test -p infrastore-core --test element_type_conformance",
            path.display()
        )
    });
