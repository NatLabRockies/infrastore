...
impl MetadataStore {
    ...
    ///
    /// Removing the last irregular series on a time axis — a
    /// `NonSequentialTimeSeries` or a `PersistentTimeSeries`, which pool onto
    /// the same axes — leaves its vector behind, since vectors are shared and a
    /// deletion cannot cascade. That is what the sweep reclaims.
